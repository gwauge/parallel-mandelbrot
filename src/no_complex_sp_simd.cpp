...
int64_t mandelbrot_computation(ofstream &matrix_out, bool output)
{
    ...
    for (uint pos = 0; pos < ALIGNED_LENGTH; ++pos) {
        const float c_re = (float)(pos % WIDTH) * (float) STEP + (float) MIN_X;
        const float c_im = (float)(pos / WIDTH)* (float) STEP + (float) MIN_Y;
        ...
        for (; iter < ITERATIONS; ++iter) {
            ...
            if (zr2 + zi2 >= 4.0f)
                break;
            float new_z_re = zr2 - zi2 + c_re;
            float new_z_im = 2.0f * z_re * z_im + c_im;
            z_re = new_z_re;
            z_im = new_z_im;
        }
        image[pos] = iter;
